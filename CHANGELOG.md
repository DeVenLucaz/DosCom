...
- **Obsolete Engines Removed**: Cleaned up the repository by permanently deleting `PoseEngine.kt` and `MovementEngine.kt`, and stripped out obsolete 2D sprite `MimeType` logic (skateboards, balloons, etc.) from `MimeEngine.kt` to optimize the codebase for the new 3D architecture.
